...
public class RobotTreasureHunt implements KeyEventDispatcher {
	...
	private void spaceBarWasPressed() {
		...
		if (robotXLocation == 730 && robotYLocation == 440) {
			JOptionPane.showMessageDialog(null, "Get the bottle in the swamp.");
		}

		// 8. Give the user subsequent clues at different locations on the image
		// (pirate robot, swamp, parrots, etc.)
		if (robotXLocation == 410 && robotYLocation == 520) {
			JOptionPane.showMessageDialog(null, "Go talk to the parrots.");
		}
		if (robotXLocation == 610 && robotYLocation == 160) {
			JOptionPane.showMessageDialog(null, "Talk to the little boy.");
		}
		if (robotXLocation == 210 && robotYLocation == 420) {
			JOptionPane.showMessageDialog(null, "The treasure is in the skull's right eye.");
		}
